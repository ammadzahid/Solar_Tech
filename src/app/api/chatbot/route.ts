...
// ─── Simple regex parser — works without Claude API ─────
function parseText(text: string): Array<{ brand: string; model: string; price: number; raw: string }> {
  const results: Array<{ brand: string; model: string; price: number; raw: string }> = []
  const lines = text.split('\n').filter(l => l.trim())

  for (const line of lines) {
    // Match patterns like:
    // "Longi 580W - 32/W"  or  "Longi: 32"  or  "Growatt 10kW 195000"
    ...
    // Extract brand/model (everything before the price indicators)
    const cleanLine = line.replace(/[-:–|]/g, ' ').trim()
    const words = cleanLine.split(/\s+/).filter(w => !w.match(/^\d/) && w.length > 1)
...
{"price_updates":[{"product_model_id":"uuid","brand":"name","model_name":"model","old_price":0,"new_price":0,"confidence":"high"}],"unrecognized":["text"]}`
        ...
        console.log('[Chatbot] Claude failed, using regex parser:', claudeErr.message)
        // Fall through to regex parser below
      }
    }

    // Regex parser fallback (or when no Claude key)
    if (priceUpdates.length === 0) {
      const parsed = parseText(text)

      for (const item of parsed) {
        // Find matching product by brand name similarity
        const match = products.find(p => {
          const pBrand = p.brand.toLowerCase()
          const pModel = p.model_name.toLowerCase()
          const itemBrand = item.brand.toLowerCase()
          return pBrand.includes(itemBrand) || itemBrand.includes(pBrand) ||
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        parser_used: (process.env.ANTHROPIC_API_KEY?.startsWith('sk-ant') ? 'claude' : 'regex'),
      }
    })

  } catch (e: any) {
    console.error('[Chatbot POST]', e)
    return NextResponse.json({ ok: false, error: e.message }, { status: 500 })
  }
}

// PUT — apply price updates to DB
